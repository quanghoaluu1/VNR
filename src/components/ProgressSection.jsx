...
function PhaseCard({ phase, index }) {
  ...
  return (
    <div ref={ref} className={`relative flex flex-col lg:flex-row items-start gap-0 ${isEven ? '' : 'lg:flex-row-reverse'}`}>
      ...
      >
        ...
        <div
          ...
          style={{ background: `${phase.color}18`, borderColor: `${phase.color}40` }}
        >
          {phase.icon}
        </div>

        <p className="font-ui text-xs tracking-[0.35em] uppercase font-semibold mb-1" style={{ color: phase.color }}>
        ...
        <div className={`h-px w-16 mb-0 ${isEven ? 'lg:ml-auto' : ''}`}
          style={{ background: `linear-gradient(${isEven ? 'to left' : 'to right'}, ${phase.color}, transparent)` }}
        />
      </motion.div>
      ...
      >
        ...
        >
          ...
          <div className="absolute top-0 left-6 right-6 h-px"
            style={{ background: `linear-gradient(90deg, transparent, ${phase.color}40, transparent)` }} />
          ...
          <div className="space-y-5">
            {phase.items.map((item, i) => (
              <div key={i} className="flex gap-3">
                ...
                <div>
                  <p className="font-ui text-lg font-semibold text-stone-200 mb-2">{item.heading}</p>
                  <p className="font-body text-base text-stone-400 leading-relaxed">{item.body}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 h-px scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"
            style={{ background: `linear-gradient(to right, ${phase.color}, transparent)` }} />
        </div>
      </motion.div>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: `${factor.color}07`, border: `1px solid ${factor.color}18` }}
      >
        {/* Top accent */}
        <div className="absolute top-0 left-4 right-4 h-px"
          style={{ background: `linear-gradient(90deg, transparent, ${factor.color}50, transparent)` }} />

        {/* Number */}
        <p className="absolute top-4 right-5 font-display text-4xl font-black leading-none"
          style={{ color: 'transparent', WebkitTextStroke: `1px ${factor.color}15` }}>
          {factor.n}
        </p>

        {/* Icon */}
        <div className="text-2xl mb-3 w-10 h-10 rounded-lg flex items-center justify-center"
          style={{ background: `${factor.color}12`, border: `1px solid ${factor.color}25` }}>
          {factor.icon}
        </div>

        <h4 className="font-ui text-lg font-semibold text-stone-200 mb-2 leading-snug pr-8">{factor.title}</h4>
        <p className="font-body text-base text-stone-500 leading-relaxed">{factor.desc}</p>

        {/* Bottom hover bar */}
        <div className="absolute bottom-0 left-0 right-0 h-px scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"
          style={{ background: `linear-gradient(to right, ${factor.color}, transparent)` }} />
      </div>
    </motion.div>
  )
}
...
export default function ProgressSection() {
  ...
  return (
    <section
      ...
      style={{ background: 'linear-gradient(to bottom, #060402 0%, #0A0805 50%, #060402 100%)' }}
    >
      {/* Background grid */}
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse, rgba(139,0,0,0.10), transparent 70%)' }} />
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div ref={headerRef} className="text-center mb-20">
          ...
          >
            Bước chuyển từ thế{' '}
            <span className="text-yellow-500/80 not-italic">"ngàn cân treo sợi tóc"</span>{' '}
            đến chiến thắng{' '}
            <span className="text-red-400/80 not-italic">"lừng lẫy năm châu, chấn động địa cầu"</span>{' '}
            — kết quả của tiến trình lãnh đạo tài tình và sáng tạo của Đảng Cộng sản Việt Nam.
          </motion.p>
        </div>
        ...
        <div className="relative">
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(to bottom, transparent 0%, rgba(201,168,76,0.25) 10%, rgba(201,168,76,0.25) 90%, transparent 100%)'
            }}
          />
        ...
        >
          <div className="inline-block relative">
            <div className="text-5xl font-display italic text-yellow-300/20 absolute -top-6 -left-4 leading-none select-none">"</div>
            <p className="font-display text-2xl md:text-3xl italic text-stone-400 max-w-2xl leading-relaxed px-8">
              Từ một chính quyền non trẻ phải đối phó với muôn vàn thách thức, Đảng đã lãnh đạo dân tộc{' '}
              <span className="text-yellow-300/80">tự lực cánh sinh</span>, từng bước chuyển từ{' '}
              <span className="text-stone-300">bị động, phòng ngự</span> sang{' '}
              <span className="text-red-400/80">chủ động tiến công</span>, đi đến thắng lợi hoàn toàn.
            </p>
            <div className="text-5xl font-display italic text-yellow-300/20 absolute -bottom-10 -right-4 leading-none select-none">"</div>
          </div>
        </motion.div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5">
            {factors.map((f, i) => (
              <FactorCard key={f.n} factor={f} index={i} />
            ))}
          </div>
        </div>
